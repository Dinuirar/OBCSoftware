...
extern SPI_HandleTypeDef SPI_TYPE;
...
{
	while (HAL_SPI_GetState(&SPI_TYPE) != HAL_SPI_STATE_READY);
	HAL_SPI_Transmit(&SPI_TYPE, &Data, 1, 5000);
}
...
static BYTE rcvr_spi(void)
{
	...
	Data = 0;
	while ((HAL_SPI_GetState(&SPI_TYPE) != HAL_SPI_STATE_READY));
	HAL_SPI_TransmitReceive(&SPI_TYPE, &Dummy, &Data, 1, 5000);
